    ...
    @else
        ...
        @else
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ 'معدل القبول' }}</span>
                    <p class="text-sm font-semibold text-success-600 dark:text-success-400">
                        {{ $accepted }}/{{ $responded }} ({{ $rates['acceptance_rate'] ?? 0 }}%)
                    </p>
                </div>
                <div>
                    <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ 'معدل عدم القبول' }}</span>
