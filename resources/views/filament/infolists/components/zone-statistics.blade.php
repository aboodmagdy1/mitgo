...
<div wire:init="loadZoneStats" class="space-y-4">
    ...
    @else
        <div class="rounded-lg border border-gray-200 dark:border-gray-700 p-4 bg-gray-50/50 dark:bg-gray-800/50">
            <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">{{ __('Total Trips Assigned') }}</h4>
        ...
        @if($totalTrips > 0)
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                    <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Complete') }}</span>
                ...
                <div class="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                    <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Acceptance') }}</span>
                    ...
                    @if($totalRequests > 0)
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ __('Of :total requests sent', ['total' => number_format($totalRequests)]) }}</p>
                    @endif
                </div>
                <div class="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                    <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Rejection') }}</span>
                ...
                <div class="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                    <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Cancellation') }}</span>
        ...
        @else
            <div class="text-sm text-gray-600 dark:text-gray-400 py-4">
                {{ __('No trips assigned to this zone yet.') }}
            </div>
        @endif
    @endif
</div>
